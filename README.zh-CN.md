...
Cortex 将记忆分为 14 个类别，每个类别有独立的重要性和衰减率：
...
| `context` | 会话上下文（系统内部） | 0.2 |
| `summary` | 压缩摘要（系统内部） | 0.4 |

### 记忆提取（Memory Sieve）

Memory Sieve 使用**双通道提取**管线：

1. **快速通道** — 基于正则表达式的模式匹配，零 LLM 延迟检测高信号信息（身份、偏好、修正、技能、关系、目标）
