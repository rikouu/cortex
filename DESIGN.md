...
### 5.2 Memory Sieve（提取+存储）

**触发点：** OpenClaw `agent:afterResponse` hook
**作用：** 从对话中提取值得记住的信息

```typescript
async function memorySieve(context: AgentContext): Promise<void> {
  const exchange = {
    user: context.lastUserMessage,
    assistant: context.lastAssistantMessage,
  };

  // 1. 高信号检测（正则，无 API 调用）
  const highSignals = detectHighSignals(exchange);
  // 高信号：修正("不是X，是Y")、偏好("我喜欢/不要")、
  //         身份声明("我是")、决策("决定/选择")、
  //         待办("需要/记得")

  // 2. 高信号立即写入 Core Memory
  if (highSignals.length > 0) {
    for (const signal of highSignals) {
      await upsertCoreMemory(signal); // 存在则更新，不存在则插入
    }
  }

  // 3. 所有内容追加到 Working Memory（轻量摘要）
  const summary = await summarizeExchange(exchange, {
    model: config.extractionModel || 'gpt-4o-mini', // API 优先，快速且便宜
    maxTokens: 200,
  });

  await appendWorkingMemory(summary);
}
```

**高信号检测不需要 LLM 调用。** 用正则和关键词检测：
